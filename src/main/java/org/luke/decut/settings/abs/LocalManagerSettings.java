...
public abstract class LocalManagerSettings extends SettingsContent {
    ...
    private final KeyedLabel defLab;
    ...
    public LocalManagerSettings(Settings settings, String name) {
        ...
        defLab = new KeyedLabel(settings.getWindow(), "default_" + name.toLowerCase() + "_version");
        ...
        addManaged.setAction(() -> addList.showPop(addManaged, Direction.DOWN_LEFT, 0, 10));
        addManaged.setCursor(Cursor.HAND);

        KeyedTooltip addVersion = new KeyedTooltip(settings.getWindow(), "add", Direction.UP, 0, 15);
        ...
        KeyedTooltip browse = new KeyedTooltip(settings.getWindow(), "browse", Direction.UP, 0, 15);
        TextTooltip.install(addLocal, browse);

        managed = new VersionsDisplay(settings.getWindow(), "managed_by_decut", addManaged);
    ...
    private ContextMenu makeContextMenu(Settings settings) {
        ...
        addList.addOnShowing(() -> {
            ...
            addList.addMenuItems(installableVersions().stream().map(v -> {
                MenuItem i = new MenuItem(addList, v, "empty")
                        .removeIcon();
                i.setAction(() -> {
