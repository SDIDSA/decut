...
public class TileHint extends Stage {
	...
	public TileHint(Window owner) {
		...
		setOnShown(_ -> owner.setAlwaysOnTop(true));

		setOnHidden(_ -> owner.setAlwaysOnTop(false));

        DoubleProperty opac = new SimpleDoubleProperty(0);
		opac.addListener((obs, ov, nv) -> setOpacity(nv.doubleValue()));

		show = new Timeline(new KeyFrame(Duration.seconds(.2), new KeyValue(opac, 1)));
		hide = new Timeline(new KeyFrame(Duration.seconds(.2), new KeyValue(opac, 0)));
	}
	...
	public State getState(double x, double y) {
		return State.stateForCoords(x - screenSize.x, y - screenSize.y, screenSize.getWidth(), screenSize.getHeight(), 100, 15);
	}
