...
public class MoveResizeHelper {
	...
	public void applyTile(Tile tile) {
		...
		parent.setPadded(false);
		if(tile.padded) {
			win.setWidth(tile.rect.getWidth());
			win.setHeight(tile.rect.getHeight());
			Platform.runAfter(() -> {
				win.setX(tile.rect.getX());
				win.setY(tile.rect.getY());
			}, 20);
		}
		win.setMaximized(!tile.padded);
