...
public class HorizontalScrollBar extends StackPane {
    ...
    public void install(Region parent, Region child) {
        thumb.widthProperty().bind(Bindings.max(40,
                parent.widthProperty().divide(child.widthProperty()).multiply(track.widthProperty())));

        child.addEventFilter(ScrollEvent.ANY, e -> {
            if(e.isControlDown()) return;
            scrollByPixels(e.getDeltaX(), child.getWidth());
        });

        prefWidthProperty().bind(parent.widthProperty());

        visibleProperty().bind(thumb.widthProperty().lessThan(track.widthProperty()));

        WeakReference<Region> weakChild = new WeakReference<>(child);
        WeakReference<Region> weakParent = new WeakReference<>(parent);
        WeakReference<HorizontalScrollBar> weakThis = new WeakReference<>(this);
