package org.luke.gui.controls.input.text;
...
public class TextInput extends Input implements Styleable, Localized {
	...
	public TextInput(Window window, Font font, String key, boolean hidden) {
		...
		field.focusedProperty().addListener((_, _, nv) -> inputStyle.focus(nv));

		caretPos = new AtomicInteger();
		field.caretPositionProperty().addListener((_, _, nv) -> {
			if (field.isFocused()) {
	...
	private void prepareMenu(Window window) {
		...
		KeyedMenuItem copy = new KeyedMenuItem(menu, "copy", "copy");
		copy.setAccelerator("ctrl+c");
		copy.setAction(this::copy);

		KeyedMenuItem cut = new KeyedMenuItem(menu, "cut", "cut");
		cut.setAccelerator("ctrl+x");
		cut.setAction(this::cut);

		KeyedMenuItem paste = new KeyedMenuItem(menu, "paste", "paste");
